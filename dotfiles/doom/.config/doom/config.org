...
- [[#auto-resize-font-on-ax-bee][auto-resize font on =ax-bee=]]
- [[#clean-up----misc-stuff][clean up -- Misc Stuff]]
- [[#ax--functions][ax-* functions]]
- [[#wip-ispell-aspell-hunspell][(wip) ispell aspell hunspell]]
- [[#janet][janet]]
- [[#calendar-setup-to-interact-with-radicale][Calendar setup to interact with radicale]]
  - [[#viewing-the-calendar-spc-j-c-c][viewing the calendar =SPC j c c=]]
  - [[#syncing-org-and-calendar-spc-j-c-s][syncing org and calendar =SPC j c s=]]
  - [[#auth-for-both-authinfo][auth for both =~/.authinfo=]]
- [[#org-publish][org-publish]]
- [[#use-dillo-to-open-urls][Use dillo to open urls]]
...
collapsed, so a fetch reveals the incoming commits inline without TAB.
#+begin_src emacs-lisp
(after! magit
...
anywhere (emacs, thunderbird, phone) and all meet on radicale. Only sync from a
...
** viewing the calendar =SPC j c c=
Opens a read-only month grid. Clearing the cache first means every call fetches
fresh, otherwise calfw keeps showing stale events even after the buffer is
killed.

The source arguments are =(url name color)=, in that order.
#+begin_src elisp
...
=org-caldav-url= is the base address, =org-caldav-calendar-id= the collection
href. New server events land in the inbox, and the listed files are read and
exported. =org-caldav-save-directory= holds the sync state and sits in ~/org so
syncthing shares it between machines.

With two machines the sync state has to match the org files it describes. If
emacs has =todo.org= open with stale text (doom only reverts on focus), the
sync pushes the old version back and undoes edits made elsewhere. Press
=SPC b r= first.
#+begin_src elisp
...
org-caldav exports via org's icalendar exporter, so these settings decide what
syncs. The default =(todo-start)= alone creates no event at all. A scheduled
time or a deadline on a TODO only becomes an event when the list holds
...
into these two lists, or a task arrives without a start and without a due
date.
...
=port http= is not a typo: url.el matches on the url scheme, not the tcp port.
Changing it to =port 5232= silently returns 401.
...
Publish with =SPC j v= (=ax/publish-site=), which forces a rebuild and drops the cached
nav. Nothing else picks up a nav edit: org republishes a page based on its own mtime and
