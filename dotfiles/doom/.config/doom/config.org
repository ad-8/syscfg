...
* Centaur Tabs
Doom only supplies the =centaur-tabs-*= faces through =doom-themes-base.el=,
which is injected exclusively into =def-doom-theme= themes. Plain =deftheme=
themes (=the-matrix=, =gotham=, the Protesilaos =doric-*=, ...) get nothing,
so centaur-tabs falls back to its hardcoded greys/black and a stale pale
filler bar. Override the faces globally to inherit from the modeline faces
every theme defines, and refresh the frozen filler color on each theme switch.
This also re-points the =def-doom-theme= themes from their =tab-bar=-derived
faces onto the modeline — uniform styling, accepted on purpose.

=amber= is the only inverted-modeline theme, so its tab strip will be solid
bright amber until a per-theme override is added.
