...
- [[#elfeed---atom-and-rss-reader][Elfeed - Atom and RSS Reader]]
  ...
  - [[#quit-an-entry-with-q][Quit an entry with q]]
...
#+end_src
*** Gotham: org-agenda done items
#+begin_src emacs-lisp
(custom-theme-set-faces!
 'gotham
 '(org-agenda-done :foreground "#245361"))
#+end_src
*** Gotham: org-habit graph
#+begin_src emacs-lisp
(custom-theme-set-faces!
 'gotham
...
#+end_src
** Quit an entry with q
Elfeed 4.0 deprecated =elfeed-kill-buffer= for =kill-current-buffer=, so
evil-collection rebound =q= to it, and doom's advice on that command leaves
...
** agenda files
Doom's default is =(list org-directory)=, which only expands to the =.org= files
directly in ~/org. This walks the tree instead, skipping dot-dirs.
#+begin_src elisp
(after! org
