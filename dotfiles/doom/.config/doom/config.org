...
** Joker
[[https://github.com/candid82/joker][Joker]] is a small Clojure interpreter, so none of the CIDER machinery above
applies — there is no nREPL to connect to. What it does have is a =joker.repl=
namespace holding =doc=, =apropos= and =dir=, a =--lint= mode, and a 9ms
startup. That last number is what keeps this section simple: every command below
shells out to =joker= and throws the process away. No session, no daemon, no
cache to invalidate.

Nothing needs installing. =clojure-mode= already ships =joker-mode= (derived
from =clojure-mode=, bound to =*.joke= and to the =#!/usr/bin/env joker=
shebang), and Doom builds its =MODE-local-vars-hook= names from the exact
=major-mode= — so a =.joke= buffer picks up neither CIDER nor =clojure-lsp=, and
nothing here can leak into the Clojure setup above.

*** helpers
Every joker namespace is preloaded, so fully-qualified symbols resolve without a
=require=. Aliases are the interesting case: re-evaluating the buffer's own =ns=
form inside the subprocess is what makes =s/join= resolve to
=joker.string/join=. That is safe — an =ns= form only runs requires, it never
touches the script body.

It can still fail, on a =:require= of a sibling file. Hence two things: =--run=
starts joker in the file's own directory, because joker resolves local requires
relative to =cwd=, and the callers retry without the prelude when it errors
anyway.
#+begin_src elisp
...
*** documentation: =K=
=K= is not bound by hand. Doom's =+lookup/documentation= consults the handlers
registered for the major mode, so registering one is all it takes.

=doc= is silent on an unknown symbol and still exits 0, so empty output is the
"nothing found" signal, and returning nil lets =+lookup/documentation= fall
through to its online search. Special forms (=if=, =recur=) document fine;
=source= and =find-doc= do not exist in joker.

Returning ='deferred= rather than =t= is load-bearing. =t= means "I moved the
cursor", and =+lookup--run-handlers= responds by restoring the window
configuration it saved beforehand — which closes the popup we just opened.
='deferred= is the value for handlers that display their own buffer.

=apropos= and =dir= feed =completing-read= rather than dumping a list into a
buffer, so vertico filters them live and =RET= goes straight to the docs. Only
=doc= gets a popup; a read-only list you cannot select from is a dead end.
#+begin_src elisp
...
*** arglists in the echo area
Same subprocess as =K=, but this one fires on every idle pause, so it has to be
async or it stutters the cursor. Unresolvable symbols print nothing and the
callback is simply never invoked, which is what eldoc wants.
#+begin_src elisp
...
A childframe package such as =eldoc-box= would render this in a floating box
instead, and was tried here. It buys presentation only, works in graphical
frames alone, and would be the single thing in this section needing a package.
The echo area carries the same text everywhere, and =K= already opens a real
popup on demand.

*** completion
Typing =os/= offers nothing on its own — corfu has no source for joker, so it
falls back to dabbrev and shows whatever words happen to be in the buffer.

One joker call produces every candidate in four shapes: namespace names, every
var fully qualified (=joker.string/join=), the =joker.core= vars that need no
prefix at all, and — via =ns-aliases= on the buffer's own =ns= form — every var
of every aliased namespace pre-written as =os/env=, =s/join=. Both the aliased
and the fully-qualified spelling have to be there; typing either one is normal
and a list holding only aliases silently offers nothing for =joker.string/=.

That is around 1030 strings fetched in 48ms, once.

The cache key is the =ns= form itself, so adding a =:require= refreshes the
candidates on the next completion and nothing else does.

Docs follow the selection, as in CIDER. =corfu-popupinfo-mode= is already on in
every corfu buffer and looks for a =company-doc-buffer= function in the capf's
plist, so supplying one is the whole job — it is called for the selected
candidate only, after corfu's half-second delay, which is why one process per
lookup stays fine. =corfu-echo= is not enabled here, so a =company-docsig=
would go unread and is left out.
#+begin_src elisp
...
*** linting
=.joke= buffers have no checker otherwise: =flycheck-clj-kondo= is disabled by
the =+lsp= flag on the clojure module, and no language server attaches here.
=global-flycheck-mode= is already on, so defining the checker is enough.

The dialect matters. Under the default =clj= dialect joker's own namespaces are
reported as unresolvable symbols — =joker.os/env= alone produces a false error.

Not using the =flycheck-joker= package on purpose: it still calls =--lintcljs=
and =--lintedn=, flags joker dropped in favour of =--dialect=, and it registers
for =clojure-mode= only, never =joker-mode=.
#+begin_src elisp
(after! flycheck
...
*** evaluating, and the repl
Registering the handlers is what wires up Doom's existing keys: =gr= over a
selection and =SPC c e= for evaluation, =SPC o r= for the repl. Results land in
the same overlay everything else uses, since =:tools (eval +overlay)= is on.

Joker prints every non-nil result, so a multi-form region reports each one.
Empty output means the region evaluated to nil.

Note the split in behaviour. =gr=, =C-x C-e= and the eval commands each spawn a
fresh process holding only the =ns= form, so a =def= from one never exists in
the next. The REPL is the stateful half: =SPC m r b= sends into a process that
persists, which is where a =def= followed by a =println= of it works.

Doom only calls a =:send-region= if the REPL window is *visible*; with the
buffer alive but its window closed it silently does nothing.
#+begin_src elisp
...
*** reformatting
=joker --format= is joker's own formatter, not cljfmt, and it reflows the whole
file — so the first run on an existing script makes a big diff. That is why it
stays a manual command rather than a format-on-save hook. Doom's
=:editor format= module is disabled anyway, which would make =set-formatter!= a
silent no-op.

=replace-buffer-contents= keeps point and markers where they were, and a
non-zero exit leaves the buffer untouched.
#+begin_src elisp
...
*** wiring it up
Kept in one =after!= block because =joker-mode-map= only exists once
=clojure-mode= has loaded.
#+begin_src elisp
(after! clojure-mode
